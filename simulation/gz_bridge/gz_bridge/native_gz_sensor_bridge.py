...
class NativeGazeboSensorBridge(Node):
    ...
    def __init__(self) -> None:
        ...
        self.bridge_clock = bool(
            self.declare_parameter("bridge_clock", True).value
        )
        self.bridge_sensors = bool(
            self.declare_parameter("bridge_sensors", True).value
        )
        if not self.bridge_clock and not self.bridge_sensors:
            raise ValueError(
                "at least one of bridge_clock or bridge_sensors must be true"
            )
        ...
        # Bridge ONLY the gimbal sensor payload.  On the gimbal landing run
        ...
        # their frames to this process.  The gimbal IMU remains available as
        # perception input.
        ...
        if self.bridge_sensors and not self.gimbal_only:
        ...
        if self.bridge_sensors:
            self._subscribe_image(
                topics["gimbal_rgb_image_gz_topic"], "gimbal_rgb")
            self._subscribe_info(
                topics["gimbal_rgb_info_gz_topic"], "gimbal_rgb_info"
            )
            # The gimbal IMU is the camera's world attitude, i.e. perception
            # input, so it is rate-limited like a camera rather than like the
            # 250 Hz raw sensor it is.
        ...
        if self.bridge_sensors and self.gimbal_only:
            self.get_logger().info(
                "gimbal_only=true — front/down RGB, depth, clouds and lidar "
                "are NOT bridged; only /gimbal_camera/* is live")
        role = (
            "clock+sensor"
            if self.bridge_clock and self.bridge_sensors
            else "clock-only" if self.bridge_clock else "sensor-only"
        )
        self.get_logger().info(
            "native Harmonic sensor bridge active "
            f"(role={role}, world={world}, model={model}, "
            f"sensor-data-on-demand="
