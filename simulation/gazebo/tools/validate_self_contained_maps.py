...
MOUNTAIN_WORLD = GAZEBO / "worlds/ugv_drone_mountain_map.world"
...
def validate_px4_contracts() -> dict[str, object]:
    ...
    require(
        'for model_dir in "$REPO_DIR"/simulation/px4_models/*/;' in linker_text
        and 'link_model "$(basename "${model_dir%/}")"' in linker_text,
        "repository PX4 model discovery contract",
    )
    ...
    require("trees=288 maze_walls=0" in generator,
...
def validate_active_uav_city_contract() -> dict[str, object]:
    ...
    require(
        ...
        and float(trailer["cruise_speed_m_s"]) == 7.0
        and float(trailer["acceleration_m_s2"]) == 9.0
        and int(trailer["start_index"]) == 0
...
def main() -> None:
    ...
    output = "\n".join(
        [
            ...
            "active_uav_city_trailer_model=moving_platform_aruco_velocity entity=trailer spawn_enu=-150,507,0 patrol_speed_m_s=7 stop_turn=1",
            ...
            f"coordinate_yaml_obstacles=active_city:{active_city['building_count']} "
            f"mountain_trees:{coordinates['mountain']} maze:0",
