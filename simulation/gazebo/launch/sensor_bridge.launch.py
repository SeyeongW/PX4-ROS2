    ...
    if backend == "native":
        ...
        clock_bridge = ExecuteProcess(
            cmd=[
                *native_cmd,
                "-p", "bridge_clock:=true",
                "-p", "bridge_sensors:=false",
                "-r", "__node:=native_gz_clock_bridge",
            ],
            name="native_gz_clock_bridge",
            output="screen",
        )
        sensor_bridge = ExecuteProcess(
            cmd=[
                *native_cmd,
                "-p", "bridge_clock:=false",
                "-p", "bridge_sensors:=true",
                "-r", "__node:=native_gz_sensor_bridge",
            ],
            name="native_gz_sensor_bridge",
            output="screen",
        )
        bridges = [clock_bridge, sensor_bridge]
    elif backend == "ros_gz":
