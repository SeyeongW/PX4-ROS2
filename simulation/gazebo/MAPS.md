...
In GUI mode this 1 km profile starts with a whole-course overview that remains
...
## 1 km forward-reverse moving-trailer shuttle

The default `mpc-landing-moving` profile is a flat, obstacle-free moving-target
landing benchmark with nominal bounds `[0,1000] x [0,100] m`. The 5 x 5 m
ArUco platform starts at `(0,50)`, drives exactly 1,000 m along ENU +x to
`(1000,50)`, and returns over the same line using negative-x velocity. Its yaw
stays at zero, so the return leg is a no-turn reverse rather than a U-turn.
The default speed is `3 m/s`, and one complete repeated cycle is 2,000 m.

The PX4 vehicle starts at `(15,40)`, initially `18.028 m` from the trailer.
There are no visual or collision obstacles. The visible experiment surface and
declared map remain exactly 1,000 x 100 m; an invisible 1,010 x 110 m collision
support underneath gives the complete trailer footprint 2.5 m of support
margin beyond each reversal plane.

The legacy world and topic name `mpc_landing_200m_moving` remains stable even
though it now identifies this 1 km shuttle experiment.

```bash
# Map-only, stationary trailer preview
...
# One complete 1,000 m forward + 1,000 m reverse validation cycle
DRIVE_TRAILER=1 TRAILER_ROUTE_LOOPS=1 \
  ./simulation/gazebo/run_px4_map.sh mpc-landing-moving
```

The preserved Cheongju University stadium variant is available separately as
`cju-track`; its source facts and attribution remain in
[`CJU_STADIUM_WORLD.md`](CJU_STADIUM_WORLD.md).

Regenerate the blue centreline and two reversal markers with:

```bash
python3 simulation/gazebo/gen_mpc_perimeter_patrol_model.py
```

The complete layout and route contract are in
[`MOVING_TRAILER_1KM_SHUTTLE_WORLD.md`](MOVING_TRAILER_1KM_SHUTTLE_WORLD.md).
...
```bash
# Mountain
GZ_SIM_RESOURCE_PATH="$PWD/gazebo/models:$PWD/gazebo/worlds" __NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia __VK_LAYER_NV_optimus=NVIDIA_only QT_QPA_PLATFORM=xcb gz sim -v4 -r --physics-engine gz-physics-bullet-featherstone-plugin "$PWD/gazebo/worlds/ugv_drone_mountain_map.world"
...
## Source assets for the active city

The former standalone 500 x 500 m world has been removed. Its source geometry
and audit assets remain because the active UAV city is derived from them.
...
- world: `gazebo/worlds/ugv_drone_mountain_map.world`
- size: 300 x 300 m
...
platform cruises at 9 m/s, starts braking 4.5 m before a stop, reaches at most
0.2 m/s, holds one zero-command cycle, changes direction, and accelerates back
to 9 m/s at 9 m/s². No circular/fillet corner mode is active. The platform is
kinematic rather than wheeled.
The mountain keeps its own optional route, using the same dependency-free
driver:

![City 9 m/s black-road trailer patrol](validation/path_planning/city_9ms_trailer_patrol.png)
