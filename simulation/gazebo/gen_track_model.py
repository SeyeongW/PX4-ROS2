#!/usr/bin/env python3
"""청주대학교 대운동장용 400 m 육상 트랙 모델 생성기.

직선 84.39 m 두 구간과 반지름 36.8 m 두 반원으로 이루어진 약 400 m
중심 경로를 사용한다. 운동장 모델과 쉽게 회전ㆍ배치할 수 있도록 장축은 로컬
x축, 경기장 중심은 로컬 원점에 둔다.
...
# 표준 400 m 트랙의 중심 경로 치수.
STRAIGHT_LENGTH_M = 84.39
CURVE_RADIUS_M = 36.8
...
<sdf version="1.9">
  ...
  <!-- Centred 8-lane visual approximation of a 400 m athletic track.
       Centre path: 2 * {STRAIGHT_LENGTH_M} m + 2 * pi * {CURVE_RADIUS_M} m.
...
def main():
    ...
    centre_perimeter = (
        2.0 * STRAIGHT_LENGTH_M + 2.0 * math.pi * CURVE_RADIUS_M
    )
    nominal_length = (
        STRAIGHT_LENGTH_M
    ...
    print(f"  centre-path perimeter: {centre_perimeter:.3f} m")
    print(
