...
def test_custom_terminal_control_parameters_are_removed():
    for name in (
            ...
            'vision_gate_height_m', 'approach_alt', 'acquire_xy_m', 'v_max'):
        assert f"'{name}'" not in MISSION_SOURCE


def test_custom_descent_controller_is_not_in_the_live_mission():
    assert 'LandingMPC(' not in MISSION_SOURCE
    ...
    assert "'DESCEND'" not in MISSION_SOURCE
