...
class MarkerKfNode(Node):
    def __init__(self):
        ...
        self.rate = float(p('rate_hz', 50.0).value)
        self.sigma_m = float(p('sigma_meas_m', 0.06).value)      # measured
        self.sigma_a = float(p('sigma_accel_m_s2', 1.5).value)   # target agility
        self.deck_z = float(p('deck_z', 1.811).value)
        # Coasting is dead reckoning: trust it for the last metre, not forever.
        self.max_coast = float(p('max_coast_s', 3.0).value)
        # A fix this far from the prediction is treated as an outlier (gating),
        # in units of the innovation standard deviation.
        self.gate_sigma = float(p('gate_sigma', 5.0).value)
        ...
        self.max_lag = float(p('max_lag_s', 0.5).value)
        self._lag_sum = 0.0
        self._lag_sq = 0.0
        self._lag_n = 0
        ...
        self.lag_jitter_floor = float(p('lag_jitter_floor_s', 0.02).value)
