...
so every marker has a geometry-defined blind height no matter how good the
tracking is.  The gauge turns that from a surprise into a readout.

Marker geometry is a LADDER configured by ``marker_sizes_m``.  Each entry is the
measured black-code side length for the matching marker ID; range scales
linearly with it, so do not change the array without re-measuring every marker.
"""
...
def maximum_pair_disagreement(points_by_id):
    """Return the largest same-frame landing-point separation and its ID pair."""
    items = list(points_by_id.items())
    worst = 0.0
    worst_pair = None
    for index, (marker_a, point_a) in enumerate(items):
        for marker_b, point_b in items[index + 1:]:
            distance = float(np.linalg.norm(
                np.asarray(point_a, dtype=float) -
                np.asarray(point_b, dtype=float)))
            if not np.isfinite(distance):
                return float('inf'), (marker_a, marker_b)
            if worst_pair is None or distance > worst:
                worst = distance
                worst_pair = (marker_a, marker_b)
...
class ArucoDetectorNode(Node):
    def __init__(self):
        ...
        self.sizes = [float(v) for v in p('marker_sizes_m',
                                          list(DEFAULT_MARKER_SIZES_M)).value]
        ...
        offs = [float(v) for v in p(
            'marker_offsets_m', list(DEFAULT_MARKER_OFFSETS_M)).value]
        validate_marker_ladder(self.ids, self.sizes, offs)
        ...
        if hasattr(cv2.aruco, 'ArucoDetector'):
            ...
            self._detect_markers = lambda image: cv2.aruco.detectMarkers(
                image, adict, parameters=params)
        ...
        self.min_px = require_positive(
            'min_marker_px', p('min_marker_px', 30.0).value)
        # Every marker on the rigid deck is reduced to the SAME landing point.
        # A larger disagreement is therefore an invalid measurement, not target
        # motion: all candidates came from one image capture.  Keep the first
        # Gazebo gate deliberately broad at 1 m, but fail closed before the KF.
        self.max_pair_disagreement = require_positive(
            'max_pair_disagreement_m',
            p('max_pair_disagreement_m',
              DEFAULT_MAX_PAIR_DISAGREEMENT_M).value)
        ...
        dump_after = float(p('dump_after_s', 4.0).value)
        self.dump_after = (
            require_nonnegative('dump_after_s', dump_after)
            if self.debug_dir else dump_after)
        self._dumped = 0
        ...
        image_topic = require_nonempty(
            'image_topic', p('image_topic', '/down_camera/image').value)
        info_topic = require_nonempty(
            'camera_info_topic',
            p('camera_info_topic', '/down_camera/camera_info').value)
        self.frame_id = require_nonempty(
            'optical_frame_id',
            p('optical_frame_id', 'down_camera_optical_frame').value)
        ...
        debug_quality = int(p('debug_image_quality', 50).value)
        self.debug_quality = (
            int(require_between(
                'debug_image_quality', debug_quality, 1, 100))
            if self.publish_debug else debug_quality)
        ...
        self.err_pub = self.create_publisher(
            ...
            require_nonempty(
                'center_error_topic',
                p('center_error_topic', '/aruco/center_error').value), 10)
        dbg_topic = str(
            p('debug_image_topic', '/aruco/debug_image').value)
        if self.publish_debug:
            dbg_topic = require_nonempty('debug_image_topic', dbg_topic)
        ...
        self.get_logger().info(
            ...
            f'pair_gate<={self.max_pair_disagreement:.2f} m')
    ...
    def _report(self):
        ...
        self.get_logger().info(
            ...
            f'pair_conflict_rejects={self._pair_conflicts}')
        ...
        if self._agree:
            ...
            if worst > self.max_pair_disagreement:
                self.get_logger().error(
                    f'the two markers disagree about the landing point by up '
                    f'to {worst:.2f} m, above the '
                    f'{self.max_pair_disagreement:.2f} m rejection threshold. '
                    f'Those frames were not published; check marker visibility, '
                    f'geometry and marker_offsets_m.')
            self._agree.clear()
    ...
    def _draw(self, img, header, poly, tvec, gap, used_id, alert=None):
    ...
    def _on_img(self, msg):
        ...
        if len(solved) > 1:
            worst, worst_pair = maximum_pair_disagreement(
                {marker_id: value[0] for marker_id, value in solved.items()})
            self._agree.append(worst)
            if worst > self.max_pair_disagreement:
                self._pair_conflicts += 1
                self.det_pub.publish(det)
                pair_text = (
                    f'id{worst_pair[0]}/id{worst_pair[1]}'
                    if worst_pair is not None else 'unknown IDs')
                self.get_logger().warn(
                    f'pair conflict rejected: {pair_text} disagree by '
                    f'{worst:.2f} m > {self.max_pair_disagreement:.2f} m',
                    throttle_duration_sec=1.0)
                if self.dbg_pub is not None:
                    marker_id = (
                        worst_pair[0] if worst_pair is not None
                        else next(iter(solved)))
                    self._draw(
                        img.copy(), msg.header, solved[marker_id][2], None, gap,
