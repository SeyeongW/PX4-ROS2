...
from .frame import (enu_dir_to_flu, flu_dir_to_enu, gimbal_axis_flu,
                    gimbal_joint_angles, gimbal_tf_chain, quat_to_rot,
                    rot_to_quat)
...
class GimbalControlNode(Node):
    def __init__(self):
        ...
        self.max_rate = math.radians(float(p('max_rate_deg_s', 90.0).value))
        self.deck_z = float(p('deck_z', 1.811).value)
        self.vision_timeout_s = float(p('vision_timeout_s', 1.0).value)
        self.cue_timeout_s = float(p('cue_timeout_s', 2.0).value)
        ...
        self.nadir_below_m = float(p('nadir_below_m', 2.0).value)
        ...
        # 30 deg is where detection starts to come back (measured: 30% at
        # 30-45 deg, 47% at 45-60), leaving margin for vision to acquire before
        # the mission's own 60 deg trust gate.  VISION, once valid, is always
        # tracked — a valid fix already implies a steep enough view.
        self.min_cue_depression = math.radians(
            float(p('min_cue_depression_deg', 30.0).value))
