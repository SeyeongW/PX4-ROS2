...
class OffboardSimRiulRTL : public rclcpp::Node {
public:
    OffboardSimRiulRTL() : Node("offboard_sim_riul_rtl") {
...
private:
    ...
    // Waypoint switching
    const float wp_switch_radius_ = 8.0f;
    ...
    // Virtual square boundary (100m x 100m)
    const float square_half_side_ = 50.0f;
        ...
        switch (phase_) {
            ...
            case Phase::move: {
                ...
                if (seg_index_ + 1 >= wp_abs_.size()) {
                    RCLCPP_INFO(this->get_logger(), "All segments done. RTL to home.");
                    start_landing_sequence();
                    break;
                }
                ...
                const float rel_x = current_pos_[0] - start_xy_[0];
                const float rel_y = current_pos_[1] - start_xy_[1];
                if (std::abs(rel_x) > square_half_side_ || std::abs(rel_y) > square_half_side_) {
                    RCLCPP_WARN(this->get_logger(),
                                "Out of virtual square (rel_x=%.1f rel_y=%.1f). RTL.",
                                rel_x, rel_y);
                    start_landing_sequence();
                    break;
                }
...
int main(int argc, char *argv[]) {
    ...
    rclcpp::spin(std::make_shared<OffboardSimRiulRTL>());
    rclcpp::shutdown();
    return 0;
}
