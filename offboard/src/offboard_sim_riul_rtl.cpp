...
class OffboardCircularTurnBoustrophedon : public rclcpp::Node {
public:
    OffboardCircularTurnBoustrophedon() : Node("offboard_circular_node") {
        ...
        odometry_subscription_ = this->create_subscription<VehicleOdometry>(
            ...
            [this](const VehicleOdometry &msg) {
                ...
                float qw = msg.q[0]; float qx = msg.q[1]; float qy = msg.q[2]; float qz = msg.q[3];
        ...
        timer_ = this->create_wall_timer(100ms, [this]() {
            if (phase_ == Phase::landing) {
                if (arming_state_ == VehicleStatus::ARMING_STATE_DISARMED) {
                    RCLCPP_INFO(this->get_logger(), "Mission End.");
                    rclcpp::shutdown();
                    return;
                }
            }
...
private:
    ...
    const float cruise_speed_ = 7.0f;
    const float wp_switch_radius_ = 10.0f; 
    const float yaw_speed_ = 1.5f;         
    ...
    void manage_mission_flow() {
        switch (phase_) {
            case Phase::warmup:
                ...
                if (arm_sent_ && ++ticks_ >= 50) phase_ = Phase::takeoff;
                break;
    ...
    void publish_trajectory_setpoint() {
        ...
        if (phase_ == Phase::move) {
            msg.velocity = {target_vel_[0], target_vel_[1], 0.0f};
        } else {
    ...
    void start_landing_sequence() {
        RCLCPP_INFO(this->get_logger(), "Mission End. RTL.");
...
int main(int argc, char *argv[]) {
    ...
    rclcpp::spin(std::make_shared<OffboardCircularTurnBoustrophedon>());
    rclcpp::shutdown();
    return 0;
}