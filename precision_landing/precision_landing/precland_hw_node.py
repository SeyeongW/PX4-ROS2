...
class PreclandHwNode(Node):
    def __init__(self):
        ...
        self._log(f'precland_hw ready — auto_takeoff={self.auto_takeoff}, '
                  f'flight_alt={self.flight_alt} m')
    ...
    def tick(self):
        ...
        if self.stage in (Stage.ALIGN, Stage.DESCEND):
            if not armed:
                ...
                self._log('GUIDED 이탈(조종사 오버라이드) -> IDLE')
                self.stage = Stage.IDLE
        ...
        if self.stage == Stage.TAKEOFF:
            ...
            if self.kf_miss > self.coast_ticks:
                self._log('마커 상실 -> IDLE'); self.stage = Stage.IDLE
            elif err is not None and marker_ok and err < self._funnel_radius():
                self._log('-> DESCEND'); self.stage = Stage.DESCEND
            ...
            if h < self.land_switch_alt and lateral < self.land_align_radius:
                ...
                self._log(f'중심 정렬({lateral:.2f} m), h={h:.2f} m -> LAND(인계)')
                self.stage = Stage.LAND
                return
            if self.kf_miss > self.coast_ticks:
                # 비전 상실: 하강 멈추고 재정렬(정지 마커라 추종할 cue 없음).
                self._log('마커 상실 -> ALIGN(고도 유지)')
                self.cmd_vel[2] = 0.0
                self.stage = Stage.ALIGN
            elif self.kf_init:
            ...
            self.land_sent += 1
            if not armed:
                self._log('접지·disarm 확인 -> 착륙 완료, 노드 종료')
                self.stage = Stage.DONE
                rclpy.shutdown()
                return
            if self.mav_state.mode != 'LAND' and self.land_sent % 20 == 1:
                self._log('set LAND'); self._set_mode('LAND')
            return                                 # 셋포인트 송출 안 함
