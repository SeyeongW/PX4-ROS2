...
Transcribed from the reference implementation in MAVProxy's SIYI module
(`MAVProxy/modules/mavproxy_SIYI/__init__.py`), which is the same protocol
ArduPilot's `AP_Mount_Siyi` speaks. Kept free of ROS and of sockets so the
framing can be checked against known vectors without a gimbal on the desk —
...
#: Command ids used by this package. The protocol has many more.
