...
def test_xy_clearance_is_round_at_aabb_corners_and_respects_z():
    world = _world(2.0)
    free = world.is_free([
        [3.0, 0.5, 0.5], [2.4, 2.4, 0.5],
        [2.5, 2.5, 0.5], [0.5, 0.5, 1.01],
    ])
...
def test_segment_uses_exact_continuous_distance_to_rounded_aabb():
    ...
    # The historical API remains sampled; exact checking is explicitly opt-in.
    assert world.segment_is_free(
        [-2.5, 2.5, 0.5], [2.5, -2.5, 0.5], step_m=100.0)
    assert not world.segment_is_free_exact(
        [-2.5, 2.5, 0.5], [2.5, -2.5, 0.5])
    assert world.segment_is_free_exact(
        [2.5, 3.0, 0.5], [3.0, 2.5, 0.5])
    assert world.segment_is_free_exact(
        [-2.5, 2.5, 1.01], [2.5, -2.5, 1.01])


def test_segment_broad_phase_keeps_tangent_zero_length_and_z_contracts():
    world = _world(2.0)
    exact = world.segment_is_free_exact
    assert not exact([3.0, -2.0, 0.5], [3.0, 3.0, 0.5])
    assert not exact([3.0, 0.5, 0.5], [3.0, 0.5, 0.5])
    assert exact([3.001, 0.5, 0.5], [3.001, 0.5, 0.5])
    assert exact([3.0, 0.5, 2.0], [3.0, 0.5, 1.001])
    assert not exact([3.0, 0.5, 2.0], [3.0, 0.5, 1.0])
...
def test_optimized_segment_math_matches_numpy_and_exhaustive_references():
    ...
    for index in range(2000):
        ...
        assert world.segment_is_free_exact(start, end) == \
...
def test_malformed_world_geometry_fails_closed():
    world = _world()
    assert not world.box_is_free([np.nan, 0.0, 0.0], [1.0, 1.0, 1.0])
    assert not world.box_is_free([2.0, 2.0, 2.0], [1.0, 1.0, 1.0])
    with pytest.raises(ValueError, match='finite ordered'):
        WorldModel.from_boxes(
            [[0.0, 0.0, 0.0]], [],
