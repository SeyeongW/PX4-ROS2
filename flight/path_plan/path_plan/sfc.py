...
class SafeFlightCorridor:
    ...
    def cover_polyline(
        ...
        This is the lightweight certificate for an already-validated active
        path.  It does not run A* or B-spline optimisation.  A
        diagonal segment whose minimum axis-aligned box is blocked is split
        until each published sub-segment fits in one free convex box.
        """
        ...
        def split(a: np.ndarray, b: np.ndarray, depth: int) -> list[np.ndarray]:
            ...
            if (np.all(hi - lo <= 2.0 * self.max_extent)
                ...
                return [a, b]
            if not self.world.segment_is_free(a, b):
        ...
        # Construction requires every published chord to lie in one free,
        # convex box.  Keep the invariant executable rather than documentary.
